...
name: Bootstrap Vue Buttons
description: Bootstrap Vue Button component for actions in tables, forms, cards, and more. Bootstrap Vue provides various styles, states, and size. Ready to use and easy to customize.
...
Bootstrap Vue includes a bunch of predefined buttons components, each serving its own semantic purpose. Buttons show what action will happen when the user clicks or touches it. Bootstrap Vue buttons are used to initialize operations, both in the background or foreground of an experience.
...
`<a>`s don't support the `disabled` attribute, so Bootstrap Vue has to add `.disabled` class to make buttons look inactive. Bootstrap Vue also has to add to the disabled button component `aria-disabled="true"` attribute to show the state of the component to assistive technologies.
